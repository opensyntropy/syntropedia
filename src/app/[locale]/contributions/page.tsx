...
import { Plus, Edit, RefreshCw, Sparkles, ImageIcon, Clock, CheckCircle, XCircle } from 'lucide-react'
...
export default async function SubmissionsPage({ params, searchParams }: SubmissionsPageProps) {
  ...
  // Check if viewing photo contributions
  const isPhotosView = status === 'PHOTOS'

  // Fetch species submissions (unless viewing photos)
  const result = isPhotosView ? null : await getSubmissions({
  ...
  // Fetch photo contributions if in photos view
  const photoContributions = isPhotosView ? await prisma.photo.findMany({
    where: userIsReviewer ? {} : { uploadById: session.user.id },
    include: {
      species: {
        select: {
          id: true,
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <Link href="/contributions?status=PHOTOS">
            <Button variant={status === 'PHOTOS' ? 'default' : 'outline'} size="sm" className="gap-1">
        ...
        {/* Photo Contributions List */}
        {isPhotosView ? (
          photoContributions.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                <p className="text-muted-foreground mb-4">{t('noPhotos') || 'No photo contributions yet'}</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {photoContributions.map((photo) => (
                <Card key={photo.id} className={`overflow-hidden ${photo.rejected ? 'border-red-200 bg-red-50/30' : ''}`}>
                  <div className="aspect-[4/3] relative">
                    <img
                      src={photo.url}
                      alt={photo.species?.scientificName || 'Photo'}
                      className={`w-full h-full object-cover ${photo.rejected ? 'opacity-60' : ''}`}
                    />
                  ...
                  <CardContent className="p-2">
                    <div className="space-y-0.5">
                      {photo.species && (
                        <Link href={`/species/${photo.species.slug}`} className="font-medium text-xs hover:underline line-clamp-1">
                          {photo.species.scientificName}
                        </Link>
                      )}
                      {photo.rejected && photo.rejectionReason && (
                        <p className="text-[10px] text-red-600 line-clamp-2">
                          {photo.rejectionReason}
                        </p>
                      )}
                      {photo.tags && photo.tags.length > 0 && !photo.rejected && (
        ...
        {/* Pagination - only for species submissions */}
        {!isPhotosView && result && result.totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-8">
            {result.page > 1 && (
              <Link href={`/contributions?page=${result.page - 1}${status ? `&status=${status}` : ''}`}>
